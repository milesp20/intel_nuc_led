...
def get_led_indicator_option(led_type, control_file=None, debug=False, quirks=None, quirks_metadata=None):
    """
    ...
    if len(list(filter(
            lambda led_indicator_option_bit: led_indicator_option_bit == '1',
            [*led_indicator_option_bitmap]
    ))) > 1:
        raise NucWmiError(
            'Error (Intel NUC WMI get_led_indicator_option function returned multiple led indicator options in bitmap)'
        )
