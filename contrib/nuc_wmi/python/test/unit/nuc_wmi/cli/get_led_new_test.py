...
class TestCliGetLedNew(unittest.TestCase):
    ...
    @patch('nuc_wmi.cli.get_led_new.load_nuc_wmi_spec')
    ...
    def test_get_led_control_item_cli( # pylint: disable=too-many-arguments,too-many-statements
        ...
        self.assertTrue(nuc_wmi.cli.get_led_new.load_nuc_wmi_spec is nuc_wmi_cli_load_nuc_wmi_spec)
    ...
    @patch('nuc_wmi.cli.get_led_new.query_led_indicator_options')
    @patch('nuc_wmi.cli.get_led_new.sys.exit')
    def test_get_led_control_item_cli12( # pylint: disable=too-many-arguments,too-many-statements
            self,
            nuc_wmi_sys_exit,
            nuc_wmi_query_led_indicator_options,
            nuc_wmi_query_led_control_items,
        ...
        # Branch 12: Test that get_led_control_item_cli raises proper error when the control file returns an invalid
        #            control item value.

        # Get HDD LED control item with invalid Brightness of HDD Activity Indicator
        control_item_value_range = defined_indexes(LED_BRIGHTNESS_MULTI_COLOR)
        ...
        self.assertEqual(
            json.loads(nuc_wmi_print.call_args.args[0]),
            {
                'error': 'Error (Intel NUC WMI get_led_control_item function returned invalid control item value of'
                         ' %i, expected one of %s)' % (expected_brightness, str(control_item_value_range))
            }
        )
    ...
    @patch('nuc_wmi.cli.get_led_new.query_led_indicator_options')
    @patch('nuc_wmi.cli.get_led_new.sys.exit')
    def test_get_led_control_item_cli13( # pylint: disable=too-many-arguments,too-many-statements
            self,
            nuc_wmi_sys_exit,
            nuc_wmi_query_led_indicator_options,
            nuc_wmi_query_led_control_items,
        ...
        # Branch 13: Test that get_led_control_item_cli raises proper error when there are no control items for
        #            for the invalid LED and indicator option chosen and returns he proper JSON error response and exit
        #            code.
        nuc_wmi_spec_alias = 'TEST_DEVICE'

        nuc_wmi_cli_load_nuc_wmi_spec.return_value = self.nuc_wmi_spec
        nuc_wmi_query_led_color_type.return_value = LED_COLOR_TYPE['new'].index('Single-color LED')
        nuc_wmi_query_led_indicator_options.return_value = [1, 4, 5,6]
